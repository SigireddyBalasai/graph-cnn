def create_model(graph, input_shape=(224, 224, 3), num_classes=100, use_mean=True, include_aux=True):
    ...
    for node in graph.nodes():
        ...
        if not predecessors:
            filters = 32
            nodes[node] = tf.keras.layers.LocallyConnected2D(filters=filters, kernel_size=graph.nodes[node]['kernel_size'], activation=graph.nodes[node]['activation'], padding='valid')(input_layer)
            ...
            nodes[node] = tf.keras.layers.Add()([nodes[node], input_layer])
